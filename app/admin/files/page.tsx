...
export default async function AdminFilesPage({ searchParams }: PageProps) {
  ...
  // Função para gerar uma badge para o tipo de arquivo
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <header className="py-8 border-b border-border/30 mb-8">
          ...
          <p className="font-mono text-muted-foreground max-w-2xl">
            Gerencie todos os arquivos relacionados aos casos investigativos
          </p>
        </header>
        ...
        <main className="py-6">
          {successType && (
            <div className="mb-8 p-4 border border-primary/30 bg-primary/5 rounded-md">
              <p className="text-primary font-mono text-sm">
                {successType === 'created' && 'Arquivo criado com sucesso!'}
                {successType === 'updated' && 'Arquivo atualizado com sucesso!'}
                {successType === 'deleted' && 'Arquivo excluído com sucesso!'}
              </p>
            </div>
          )}
          ...
          {files.length === 0 ? (
            <div className="text-center p-10 border border-border/40 rounded-lg bg-muted/50">
              <h2 className="investigative-title text-xl font-medium mb-2">NENHUM ARQUIVO ENCONTRADO</h2>
              <p className="text-muted-foreground mb-6 font-mono">
                Não há arquivos investigativos cadastrados no sistema.
              </p>
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-muted/30 hover:bg-muted/30">
                    ...
                    <TableHead className="font-mono text-xs">DISPONÍVEL EM</TableHead>
                    <TableHead className="font-mono text-xs text-right">AÇÕES</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {files.map((file) => (
                    <TableRow key={file.id} className="font-mono text-sm">
                      <TableCell className="font-medium">{file.title}</TableCell>
                      <TableCell>
                        <Badge variant={getTypeBadge(file.type)} className="font-mono">
                          {file.type}
                        </Badge>
                      </TableCell>
                      <TableCell>
                        {file.case?.title || "Sem caso"}
                      </TableCell>
                      <TableCell className="text-muted-foreground">
                        {new Date(file.availableAt).toLocaleDateString('pt-BR')}
                      </TableCell>
                          ...
                          <Dialog>
                            ...
                            <DialogContent className="sm:max-w-md font-mono">
                              <DialogHeader>
                                ...
                                <DialogDescription>
                                  Tem certeza que deseja excluir o arquivo "{file.title}"? 
